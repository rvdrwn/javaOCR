...
def initialize_parameters():
    ...
    W1 = tf.compat.v1.get_variable("W1", [4,4,1,8], initializer = tf.compat.v1.keras.initializers.VarianceScaling(scale=1.0, mode="fan_avg", distribution="uniform", seed = 0))
...
def forward_propagation(X, keep_prob):
    ...
    # CONV 1
    W1 = tf.compat.v1.get_variable("W1", [4,4,1,64], initializer = tf.compat.v1.keras.initializers.VarianceScaling(scale=1.0, mode="fan_avg", distribution="uniform", seed = 0))
    ...
    print(A1)
    # MAX POOL 1
    P1 = tf.nn.max_pool2d(input=A1, ksize = [1,4,4,1], strides = [1,4,4,1], padding = "SAME")
    print(P1)

    # CONV 2
    W2 = tf.compat.v1.get_variable("W2", [2,2,64,128], initializer = tf.compat.v1.keras.initializers.VarianceScaling(scale=1.0, mode="fan_avg", distribution="uniform", seed = 0))
    b2 = tf.compat.v1.get_variable("b2", [128,1], initializer = tf.compat.v1.zeros_initializer())
    ...
    print(P2)
    # FULL CONNECT 1
    W3 = tf.compat.v1.get_variable('W3', [512, P2.shape[1:2].num_elements()], initializer = tf.compat.v1.keras.initializers.VarianceScaling(scale=1.0, mode="fan_avg", distribution="uniform", seed=0))
    b3 = tf.compat.v1.get_variable('b3', [512,1], initializer = tf.compat.v1.zeros_initializer())
    Z3 = tf.add(tf.matmul(W3,tf.linalg.matrix_transpose(P2)), b3)
    A3 = tf.nn.relu(Z3)
    print(A3)
    # FULL CONNECT 2
    W4 = tf.compat.v1.get_variable('W4', [256, 512], initializer = tf.compat.v1.keras.initializers.VarianceScaling(scale=1.0, mode="fan_avg", distribution="uniform", seed=0))
    b4 = tf.compat.v1.get_variable('b4', [256,1], initializer = tf.compat.v1.zeros_initializer())
    A4_drop = tf.nn.dropout(A3, 1 - (keep_prob))
    Z4 = tf.add(tf.matmul(W4,A4_drop), b4)
    A4 = tf.nn.relu(Z4)
    print(A4)
    # FULL CONNECT 3
    W5 = tf.compat.v1.get_variable('W5', [63,256], initializer = tf.compat.v1.keras.initializers.VarianceScaling(scale=1.0, mode="fan_avg", distribution="uniform", seed=0))
    b5 = tf.compat.v1.get_variable('b5', [63,1], initializer = tf.compat.v1.zeros_initializer())
    A5_drop = tf.nn.dropout(A4, 1 - (keep_prob))
    Z5 = tf.add(tf.matmul(W5,A5_drop), b5)
    print(Z5)

    Z5 = tf.linalg.matrix_transpose(Z5)

    return Z5

def compute_cost(Z3, Y):
    cost = tf.reduce_mean(input_tensor=tf.nn.softmax_cross_entropy_with_logits(logits = Z3, labels = Y))
...
def model(X_train, Y_train, X_test, Y_test, learning_rate = 0.001, keep_prob_prob = 1,
	...
	y_pred_class = tf.argmax(input=y_pred, axis = 1)
	y_true_class = tf.argmax(input=Y, axis = 1)
...
if __name__ == "__main__":
	train_inputs, train_outputs , test_inputs, test_outputs = np.load('training_inputs_java.npy'), np.load('training_outputs_java.npy'), np.load('test_inputs_java.npy'), np.load('test_outputs_java.npy')
